 ...
 * {@link CondFreqDist} can be used as follows:
 * 
 * <p><blockquote><pre>
 * {@code
 ...
 * </pre></blockquote>
 * 
 * </blockquote> The last call to {@link CondFreqDist#getCount} will yield 2,
...
public class CondFreqDist<C, V>
{
    ...
    private Map<C, FreqDist<V>> cfd;

    /** The total number of samples of all FrequencyDistributions. */
    private long n;

    /**
     * Creates a new empty {@link CondFreqDist}.
     */
    public CondFreqDist()
    {
        cfd = new HashMap<C, FreqDist<V>>();
        n = 0;
    }

    /**
     * Creates a new {@link CondFreqDist} and fills it with samples from a map.
     * 
     * @param samples
    ...
    /**
     * Returns the {@link FreqDist} under a given <code>condition</code>, or
     * <code>null</code> if this distribution contains no such {@link FreqDist} for
     ...
     */
    public FreqDist<V> getFrequencyDistribution(C condition)
    {
     ...
     */
    public void setFrequencyDistribution(C condition, FreqDist<V> fd) {
     ...
     * 
     * <p>
     * If there is no {@link FreqDist} present for the given <code>condition</code>, a
    ...
    {
        FreqDist<V> freqDist = null;

        if (cfd.containsKey(condition)) {
            freqDist = cfd.get(condition);
        }
        else {
            freqDist = new FreqDist<V>();
